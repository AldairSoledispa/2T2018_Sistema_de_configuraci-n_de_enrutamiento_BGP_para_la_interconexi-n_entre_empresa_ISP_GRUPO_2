...
def inicio(conexion):
    ...
    frame=Frame(root,bg="ivory2",width=600,height=480,pady=300,padx=100)
    frame.pack()
        
    #Etiqueta de titulo
    titulo_label=Label(frame,text="ASISTENTE PARA CONFIGURACION BGP",fg="#0431B4",font=("Helvetica", 13))

    titulo_label.place(x=-20,y=-70,)

    #Campo de formulario del usuario
    usuario=Entry(frame)
    usuario.grid(row=1,column=2, sticky="e", padx=10, pady=10)
    usuario_label=Label(frame,text="Usuario: ",font=("Helvetica", 13))
    ...
    passwd_label=Label(frame,text="Contraseña: ",font=("Helvetica", 13))
...
def ventana_dispositivos(conexion):
    ...
    frame=Frame(root,bg="ivory2",width=600,height=480,pady=300,padx=75)
    frame.pack()

    #etiqueta del titulo   
    titulo_label=Label(frame,text="ESCOGER DISPOSITIVOS",fg="#0431B4",font=("Helvetica", 12))
    titulo_label.place(x=225,y=-70)
    ...
    combo_dispositivoL["values"]=[] ## dispositivos por empresa
    ...
    boton.grid(row=4,column=3,padx=0, pady=50)
